...
import { computeAggregateInheritedTags, type InheritedTagRow } from "@/lib/tag-inheritance";
...
export function MetadataContent({
  ...
  const aggregateInheritedTagRows = useMemo<InheritedTagRow[]>(() => {
    return computeAggregateInheritedTags(selectedEntriesForMetadata, tagById, tagDisplayContext);
  }, [selectedEntriesForMetadata, tagById, tagDisplayContext]);
      ...
      <div className="metadata-collapsible-section">
        <div className="metadata-section-header">
          ...
          >
            ...
            {aggregateInheritedTagRows.length > 0 ? (
              <span className="metadata-section-count">({aggregateInheritedTagRows.length})</span>
            ) : null}
          </button>
        </div>
        ...
        {inheritedTagsOpen ? (
          <div className="metadata-tag-list">
            {aggregateInheritedTagRows.length === 0 ? (
              ...
              aggregateInheritedTagRows.map((row) => (
                <div
                  key={row.tagId}
                  className="metadata-tag-chip"
                  style={row.tag ? resolveTagChipStyle(row.tag, tagColorLookup) : undefined}
